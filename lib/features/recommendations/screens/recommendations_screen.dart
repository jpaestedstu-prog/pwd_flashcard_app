        ...
        child: CustomScrollView(
          slivers: [
            ...
            if (snapshot.recommendations.isEmpty)
              SliverToBoxAdapter(
                child: _AllCaughtUp(isFilipino: isFilipino),
              )
            else
              SliverPadding(
...
class _RecommendationCard extends ConsumerWidget {
  ...
  void _navigate(BuildContext context, WidgetRef ref) {
    ...
    try {
      ...
      // Use context.go() (not push) because these routes live inside
      // the ShellRoute and we are navigating from outside of it.
      if (params != null && params.isNotEmpty) {
        final queryString =
            params.entries.map((e) => '${e.key}=${e.value}').join('&');
        context.go('$route?$queryString');
      } else {
        context.go(route);
      }
    } catch (e) {
